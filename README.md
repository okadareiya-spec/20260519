...
FastAPI /webhook エンドポイント
    ...
    │  3. BackgroundTasks に _handle_message() を登録
    │  ※ 即座に 200 OK を返す（LINE の 5 秒タイムアウト対策）
    ▼
_handle_message()（バックグラウンド実行）
    │  4. リッチメニューコマンドの判定
    │     SWITCH_TEACHER  → ロール切替（履歴リセット）＋先生役ウェルカム返信
    │     SWITCH_ADVISOR  → ロール切替（履歴リセット）＋相談役ウェルカム返信
    │     END_CONVERSATION → 履歴削除・返信なし
    │  5. Upstash Redis からロールと当日の会話履歴を取得
    │     会話キー: lion:conv:{user_id}:{YYYY-MM-DD}
    │     ロールキー: lion:role:{user_id}
    │     ※ 当日キーがなければ空リスト → 利用案内＋ウェルカムメッセージ＋AI返答を同時送信
    │  6. ユーザー発言を履歴に追加
    ▼
Claude API（claude-sonnet-4-6）
    ...
    │
    │  【相談役モードの回答フロー】
    │     フェーズⅠ：情報収集（最大6ターン・延長可、A/B形式）
    │       目的 / あるべき姿 / 現状の問題 / 原因 を収集
    │       6ターン到達時：「もう少し話したい A/B」で確認 → A選択でカウントリセット・継続
    │     フェーズⅡ：認識確認（4要素を提示して A/B で合意）
    │     フェーズⅢ：解決策提案
    │       ① 課題設定（1文）
    │       ② 示唆とアクション（原則の考え方 → なぜこの行動が有効か、の因果で展開）
    │       ③ 原則との橋渡し（提案したロジックの源泉が原則に詰まっていることを明示）
    │          ＋「資料 P.[ページ番号]：[URL]」を末尾に付記（文字数カウント外）
    │
    │  【先生役モードの回答フロー】
...
LINE Messaging API（Reply API）
    │  9. ユーザーへ返信（新セッション時は利用案内＋ウェルカム＋AI返答を1回で送信）
    ▼
ユーザー（LINE）
```
...
| 相談役の応答フロー | 最大6ターン情報収集（6回到達時に延長確認あり）→ 認識確認 → 解決策提案（課題設定＋原則起点の論理展開＋原則との橋渡し）。解決策提案後にカウントリセット |
| 先生役の応答フロー | 原則の説明＋資料URL＋ページ番号 |
| 質問形式 | すべての質問は A/B 2択形式（リッチメニューで回答） |
| 会話終了 | リッチメニュー「会話終了」ボタン or クローズサイン検知 |
| 応答文字数 | 相談役フェーズⅢ：200〜400文字（資料URL・ページ番号行はカウント外）、先生役：300〜500文字 |
| 資料URL管理 | `system_prompt.py` の `CORE_OF_CORE_URL` で一元管理。相談役・先生役ともに同一URLを参照 |
| 会話履歴 | 当日中のみ保持（日付をまたぐと新セッション） |
| 履歴上限 | 直近20ターン |
| ロール保持 | 1週間（日付またいでも維持） |
| 利用案内通知 | ウェルカムメッセージ送信時（ロール切替・新セッション）に `USAGE_NOTICE` を別バブルで先送り（コールドスタート時の再送を案内） |
| Webhook応答 | 署名検証後すぐに 200 OK を返し、メッセージ処理は BackgroundTasks で非同期実行（LINE 5秒タイムアウト対策） |
